    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const res = await fetch(
                `http://192.168.18.19:8000/api/reviews/${id}/`,
                {
                    method: "PATCH",
                    credentials: "include",
                    headers: {
...
export const getReviewDetails = (id) => {
    return async (dispatch, getState) => {
        ...
        dispatch({ type: "GET_REVIEW_DETAILS" });
        try {
            const tokens = await AsyncStorage.getItem("tokens");
            const { access } = JSON.parse(tokens);
            ...
            console.log('review details success get')
            dispatch({ type: "GET_REVIEW_DETAILS_SUCCESS", payload: data });
        } catch (e) {
            console.log(e);
            dispatch({
